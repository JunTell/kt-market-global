...
export default function OrderUserForm(props: OrderUserFormProps) {
  ...
  }, [props.planName, props.planData, t])
    ...
    return (
      <div className="w-full max-w-[480px] mx-auto min-h-screen z-50 fixed top-0 left-1/2 -translate-x-1/2 bottom-0 overflow-y-auto bg-white animate-fadeInUp">
        ...
        <div className="flex flex-col px-5">
          <InputGroup label={t('Phone.OrderForm.name_label')} value={formData.userName} onChange={(e:any) => handleChange("userName", e.target.value)} />
          <InputGroup
          ...
          <InputGroup label={t('Phone.OrderForm.requirements_label')} placeholder={t('Phone.OrderForm.requirements_placeholder')} value={formData.requirements} onChange={(e:any) => handleChange("requirements", e.target.value)} />
        </div>
...
const InfoRow = ({ label, value, hasButton = false, onEdit, buttonLabel = "수정" }: any) => (
  <div className="flex items-start justify-between">
    ...
    <div className="flex items-center gap-2.5 flex-1">
      <span className="text-[15px] font-semibold text-[#1d1d1f] leading-snug">{value}</span>
